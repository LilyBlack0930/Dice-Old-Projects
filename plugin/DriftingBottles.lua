...
/ >❤     依据个人喜好，合并了扔瓶子与书写留言的步骤，原插件是让用户分两段发送的。
		  ...
		  v1.3更新日志：漂流瓶重置支持@用户，在群聊内无需手动输入QQ号。修复了“漂流瓶重置”后跟不识别内容后会报错的状况，原先版本仅会于0级窗口报错，不影响正常使用。
		  v1.4更新日志：增加监听功能，默认关闭，需要手动设置QQ号或群号或通知窗口编号。
		  v1.5更新日志：增加禁止小窗功能，优先级高于监听，会无视小窗发送的漂流瓶系列消息
]]
msg_order = {}
...
-- 监听模式，开启后将在设定位置同步每一条漂流瓶的内容，推荐设置在小群并屏蔽通知或设置在骰娘自身聊天窗口。
monitor_mode = 2 --1为群聊，2为私聊，3为通知窗口

-- 监听模式开启下发送窗口位置，默认为骰娘自身QQ
monitor_subject = getDiceQQ()
-- monitor_subject = "1142145792" 账号、群号示例

-- 监听模式下通知窗口位置
monitor_notice = "0" -- 默认0级，可用数字0-9，6-9级需要手动通过【.admin notice +X】开启，其中6级由于可能与每日问安冲突因此不推荐
...
-- 每日扔瓶子的上限
DB_everyday_max_throw = 20

-- 每日捡瓶子的上限
DB_everyday_max_pick = 20

--每日跳海的上限
DB_everyday_max_drown = 1

-- 骰主的QQ号，请务必更改否则你的重置权限可就归兔兔了（
master_qq = "1142145792"
...
function throw_bottle(msg)
	...
	if ( getUserToday(msg.fromQQ,"DB_everyday_throw",0)>= DB_everyday_max_throw ) then
		...
		if ( msg.fromGroup == "0" ) then
			...
			monitor_message = os.date("%Y-%m-%d %H:%M:%S") .. "\n用户"..getUserConf(msg.fromQQ,"nick","(获取用户名信息失败！)").."("..msg.fromQQ..")在骰娘的私聊窗口扔下一个漂流瓶：\n"..letter
		else
			letter_full = "你在海边捡到了一个"..random_bottle.."，打开瓶子，里面有一张纸条，写着：\n"..letter.."\n【来自群："..getGroupConf(msg.fromGroup, "name", "获取群名参数错误！").."("..msg.fromGroup..")的"..getUserConf(msg.fromQQ,"nick","某人").."("..msg.fromQQ..")留下的漂流瓶】c2xhc2g="
			monitor_message = os.date("%Y-%m-%d %H:%M:%S") .. "\n用户"..getUserConf(msg.fromQQ,"nick","(获取用户名信息失败！)").."("..msg.fromQQ..")在群"..getGroupConf(msg.fromGroup, "name", "获取群名参数错误！").."("..msg.fromGroup..")扔下一个漂流瓶：\n"..letter
		end
		-- 写入本地
		...
		-- 监听模式下向指定窗口发送消息
		if monitor_mode==1 then -- 群
			sendMsg(monitor_message,monitor_subject,0)
		elseif monitor_mode==2 then -- 私聊
			sendMsg(monitor_message,0,monitor_subject)
		elseif monitor_mode==3 then	-- 广播
			eventMsg(".send notice "..monitor_notice.." "..monitor_message,0,getDiceQQ())
		end
...
msg_order["拣漂流瓶"] = "pick_bottle"
msg_order["捞漂流瓶"] = "pick_bottle"
msg_order["掏漂流瓶"] = "pick_bottle"
msg_order["拾漂流瓶"] = "pick_bottle"
...
msg_order["跳入海里"] = "bottle_num"
msg_order["沉入海里"] = "bottle_num"
msg_order["沉入大海"] = "bottle_num"
msg_order["进入大海"] = "bottle_num"
msg_order["跳入海中"] = "bottle_num"
msg_order["跳进海中"] = "bottle_num"
msg_order["沉入海中"] = "bottle_num"
msg_order["进入海中"] = "bottle_num"
msg_order["跳入深海"] = "bottle_num"
msg_order["跳进深海"] = "bottle_num"
msg_order["沉入深海"] = "bottle_num"
msg_order["进入深海"] = "bottle_num"
...
function DB_reset(msg)
	...
	if ( msg.fromQQ == master_qq ) then
		local user_qq = string.match(msg.fromMsg,"%d+",#reset_order+1)
		local a = #Split(msg.fromMsg,"%d+")
		if a == 1 then
			return "请输入需要重置的目标的QQ号，或直接在群内@对方以进行重置！"
		end
